...
1. 在 post 目录下新建一个文件夹用于存放文章的 Markdown 文件和插图文件，文件夹可根据喜好随意命名，我个人一般按照日期或年份命名。
2. 在刚建立的文件夹下新建一个以日期加文章标题格式的 Markdown 文件如：`2023-09-21-搭建个人博客.md`。Hugo 生成器对原始文件名并非强制要求这样，但是这样便于管理。
